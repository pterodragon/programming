...
std::vector<TestFilePaths> get_all_test_filepaths(
    const bf::path &target_path, const std::string &input_prefix,
    ...
    for (auto dir_it : bf::directory_iterator(target_path)) {
        if (!bf::is_regular_file(dir_it.status())) continue;
    ...
    for (const auto &input_path : input_paths) {
        ...
        const auto &output_path_it =
            output_paths.find(corresponding_output_path);
