...
public:
  vector<pair<int, int>> pacificAtlantic(const vector<vector<int>> &matrix) {
    ...
    vector<vector<bitset<2>>> pa(
        matrix.size(),
        vector<bitset<2>>(matrix[0].size(),
                          0)); // first and second bit represents "can go to
                               // Pacific/Atlantic ocean" resp.
  ...
  void flood(size_t i, size_t j, const vector<vector<int>> &matrix,
             vector<vector<bitset<2>>> &pa, unsigned ocean_bit) {
    if (pa[i][j][ocean_bit])
      return; // visited
    pa[i][j][ocean_bit] = 1;
