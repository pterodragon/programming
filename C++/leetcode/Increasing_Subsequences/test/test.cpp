...
{
    Solution sol;
    std::unordered_set<std::vector<int>, boost::hash<std::vector<int>>> set_output;
    std::unordered_set<std::vector<int>, boost::hash<std::vector<int>>> set_actual;
    for (const auto& e : test_data.output) {
        set_output.insert(e);
    }
    auto actual = sol.findSubsequences(const_cast<vector<int>&>(test_data.input));
    for (const auto& e : actual) {
        set_actual.insert(e);
    }
    BOOST_CHECK_EQUAL(actual.size(), test_data.output.size()); // test_data.output is unique; actual result must be unique too
