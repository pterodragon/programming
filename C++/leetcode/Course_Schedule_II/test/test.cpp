...
bool check_actual(int numCourses, const vector<pair<int, int>> &prerequisites,
                  const vector<int> &actual) {
  if (numCourses != actual.size())
    return false;
  struct deg_prop {
    int in_degree = 0;
    int out_degree = 0;
...
BOOST_DATA_TEST_CASE(test1, (NumberedTestsFromFiles(&read_input, &read_output)),
                     ...
                     test_data) {
  Solution sol;
  auto actual = sol.findOrder(
      test_data.input.nCourses,
      const_cast<Input::PrereqPairs &>(test_data.input.prereqPairs));
  // BOOST_CHECK_EQUAL(actual, test_data.output);
  BOOST_CHECK(check_actual(test_data.input.nCourses,
                           test_data.input.prereqPairs, actual));
}
